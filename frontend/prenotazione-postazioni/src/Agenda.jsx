...
export const Agenda = ({ giorno, setGiorno, prenotazioni }) => {
    ...
    useEffect(() => {
        if (prenotazioni) {
            const uniqueUserIds = new Set(prenotazioni.map((prenotazione) => prenotazione.createUserId));
            ...
            <div className='overflow-y-auto verocard'>
                <div>
                    {cardData.map((cardItem) => (
                        <div className="card btn carta pl-0" key={cardItem.id}>
                            <div className="card-body d-flex">
                                <div className='col-md-8'>
                                    <p className="card-title font-weight-bold h5 text-left" style={{ color: "#3AA4B3" }}>{cardItem.id}</p>
                                    <p className="card-text mb-2 text-muted">Creata da {createUser?.username}</p>
                                </div>
                                <div className='col-md-4'>
                                    <button type="button" className="btn btn-sm me-1 btn-info" onClick={() => alert(" modificato")}>
                                        <svg xmlns="http://www.w3.org/2000/svg" width="15" height="15" fill="#729da3" class="bi bi-pencil-fill" viewBox="0 0 16 16">
